...
#------------------------------------------VALOR RGB PARA IMPRIMIR UMA COR A CADA PONTO-------------------------------
	...
	#----------------------condi��o para saber que configura��o de dire��o para realizar o desenho do retangulo------------------------
	slt $t7, $s1,$s3  
	bne $t7, $zero,LOOP1
	beq $t7, $zero,LOOP4
...
LOOP4:	
	...
	#SAIR6 para variar a coluna(y---------------->) e manter o (x)  
		sair7:		
			sw $t5, 0($t0)
			addi $t0, $t0, 4
	...
	#sair7 para variar a linha (x) de cima para baixo
			sair8:	
  				sw $t5, 0($t0)
				addi $t0, $t0, -256
